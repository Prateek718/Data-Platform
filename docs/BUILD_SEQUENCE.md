...
| 11 | **README & disclosure** | Project README + honest disclosure of limitations, scope boundaries, and known data caveats. | ✅ done — the README is the front door: every factual claim in it (and in REPRODUCIBILITY.md) audited against the committed artifacts, with four errors corrected; structured for the three readers (the reading, the served record, the architecture); one authoritative **Scope, coverage and caveats** section pointing at the report's Limitations as the full statement; and a test asserting every documented `data-platform-*` command is a declared console script. |
